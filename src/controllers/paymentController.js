  ...
  createPayment: async (req, res, next) => {
    try {
      ...
      if (result.gateway === 'paddle' && plan.payment_type === 'recurring') {
        ...
        // ✅ FIXED: Return direct Paddle checkout URL (not internal page URL)
        return res.json({
          success: true,
          type: 'recurring',
          paymentData: {
            transaction_id: paddleResult.transaction_id,
            checkout_url: paddleResult.checkout_url, // ✅ Direct Paddle checkout URL
          },
          gateway: 'paddle',
      ...
      // ✅ Regular Stripe/Paddle one-time payment
      res.json(result);
    } catch (error) {
...
//         const paddlePageUrl = `${frontendUrl}/payment/paddle?plan_id=${planId}&price_id=${plan.paddle_price_id}&plan_name=${encodeURIComponent(plan.plan_name)}&price=${plan.price}&billing_cycle=${plan.billing_cycle}`;
