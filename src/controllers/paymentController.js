...
createPayment: async (req, res, next) => {
    console.log('🚀 PAYMENT CONTROLLER VERSION: 2025-10-28-FINAL');
    const { amount, currency, payment_method, country_code, region, plan_id } = req.body;
    console.log('🔍 CONTROLLER - Extracted plan_id:', plan_id);
    ...
    try {
      ...
      const plan = await subscriptionQueries.getPlanById(plan_id);
      ...
      console.log(`   Plan: ${planData.plan_name} (ID: ${plan_id})`);
      console.log(`   Type: ${planData.payment_type}`);
      console.log(`   Country: ${country_code}`);
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');

      // ✅ MODIFIED: Handle pay-as-you-go - route to gateway instead of immediate activation
      if (planData.payment_type === 'pay_as_you_go') {
        console.log('→ Pay-as-you-go plan - routing to payment gateway');
        
        // Route to gateway for one-time payment (same as other plans)
        const paymentResult = await gatewayRouterService.createPaymentByCountry({
        ...
        if (!paymentResult.success) {
          return res.status(400).json({
            error: paymentResult.error || 'Payment gateway routing failed',
...
const regionData = { region: recommendation.region };
      ...
      finalAmount = parseFloat(finalAmount) + parseFloat(processingFee);

      // For recurring plans, route through gateway router
