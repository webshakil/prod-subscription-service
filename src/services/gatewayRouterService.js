  ...
  createPaymentByCountry: async (data) => {
    try {
      ...
      if (plan_id) {
        ...
        // ✅ Allow pay-as-you-go to go through gateway for one-time payment
        if (planDetails?.payment_type === 'pay_as_you_go') {
          console.log('💳 Pay-as-you-go plan detected - routing for one-time payment');
        }
      }
      ...
      if (gatewaySelection.gateway === 'stripe') {
        ...
        // ✅ PADDLE - MODIFIED SECTION ONLY
        console.log('→ Routing to Paddle (2025 API)');

        const paddleService = await import('./paddleService.js');

        // ✅ NEW: Check if recurring or one-time
        const isRecurring = planDetails?.is_recurring || 
                           (planDetails?.payment_type !== 'pay_as_you_go' && planDetails?.duration_days > 0);

        console.log(`   Plan type: ${isRecurring ? 'RECURRING' : 'ONE-TIME'}`);
        console.log(`   Plan name: ${planDetails?.plan_name}`);

        if (isRecurring) {
          // Recurring subscription
          console.log('   → Creating Paddle SUBSCRIPTION');
          paymentResult = await paddleService.paddleService.createSubscription({
            user_id: paymentData.user_id,
            email: paymentData.email,
            planId: plan_id,
          });
        } else {
          // One-time payment (pay-as-you-go)
